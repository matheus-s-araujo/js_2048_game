...
startButton.addEventListener("click", ()=>{
    if (startButton.classList.contains("start")) handleStart();
    else handleRestart();
});
...
class Game {
    ...
    addNewNumber() {
        const empty = [];
        for(let i = 0; i < 4; i += 1){
            for(let j = 0; j < 4; j += 1)if (this.board[i][j] === 0) empty.push({
                i,
                j
            });
        }
    ...
    compressAndMerge(line) {
        ...
        const newLine = [];
        let scoreGain = 0;
        for(let i = 0; i < filtered.length; i += 1)if (filtered[i] === filtered[i + 1]) {
    ...
    moveLeftInternal() {
        ...
        for(let i = 0; i < 4; i += 1){
            ...
            if (newLine.toString() !== this.board[i].toString()) moved = true;
    ...
    moveRightInternal() {
        ...
        for(let i = 0; i < 4; i += 1){
            ...
            if (finalLine.toString() !== this.board[i].toString()) moved = true;
    ...
    moveUpInternal() {
        ...
        for(let col = 0; col < 4; col += 1){
            ...
            for(let row = 0; row < 4; row += 1){
                if (this.board[row][col] !== newLine[row]) moved = true;
    ...
    moveDownInternal() {
        ...
        for(let col = 0; col < 4; col += 1){
            ...
            for(let row = 0; row < 4; row += 1){
                if (this.board[row][col] !== finalCol[row]) moved = true;
